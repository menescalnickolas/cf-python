...
class NewRecipe(Base):
  ...
  def calculate_difficulty(self):
    ...
    if self.cooking_time < 10 and ingredients < 4:
        self.difficulty = "Easy"
    elif self.cooking_time < 10 and ingredients >= 4:
...
def create_recipe():
  ...
  print("Recipe has been added to the list of recipes!")


def view_all_recipes():
   #Retrieves all recipes from the database as a list
   recipes_list = session.query(NewRecipe).all()
...
def search_by_ingredients():
  ...
  recipe_count = session.query(NewRecipe).count()
  ...
  results = session.query(NewRecipe.ingredients).all()
  ...
  for i in search_ingredients:
    ...
    conditions.append(NewRecipe.ingredients.like(like_term)) #Appends the search condition with the like_term

      #Retrieves all recipes from database using filter() that contains the conditions, and displays the recipes
  results = session.query(NewRecipe).filter(*conditions).all()
...
def edit_recipe():
  ...
  results = session.query(NewRecipe.id, NewRecipe.name).all()
  ...
  recipe_to_edit = session.query(NewRecipe).filter_by(id=int(selected_recipe)).one()
  ...
  if choice == "1":
   ...
   #If user chooses to edit recipe's ingredients
    ...
    for i in range (number_of_ingredients):
      ingredient = input("Enter the name of the ingredient: ")
      edited_ingredients.append(ingredient) #Adds ingredient to the temporary list
...
def delete_recipe():
   ...
   #Retrieves id and name for each recipe and stores them in "results"
   recipes_list = session.query(NewRecipe.id, NewRecipe.name).all()

   print("Recipes available to be deleted: ")
   
   for recipe in recipes_list:
    print(f"ID: {recipe.id} - Name: {recipe.name}") #Display recipes to the user
    
  #Lets user pick a recipe by ID - if ID is invalid, exit the function
   selected_recipe = input("Enter the ID of the recipe you'd like to delete: ")
   if not selected_recipe.isnumeric() or int(selected_recipe) not in [r.id for r in recipes_list]:
       print("The ID is invalid!")
       return None
    
  #Retrieves recipe that corresponds with the selected ID
   recipe_to_delete = session.query(NewRecipe).filter_by(id=int(selected_recipe)).one()
   print(recipe_to_delete)

  #Confirm with the user that this is the right recipe to be deleted
   choice = input(f"Are you sure you want to delete '{recipe_to_delete.name}'? Enter 'yes' or 'no': ").lower()

   if choice == 'no':
        print("We will not proceed with deletion.")
        return None
        ...
        print("Enter 'yes' or 'no' to delete recipe!")
        return None
